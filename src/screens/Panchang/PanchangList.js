import {
  ...
  const PanchangList = ({navigation, customerData}) => {
      ...
      const renderItem = ({item, index}) => {
        ...
        return (
          <TouchableOpacity
            activeOpacity={0.8}
            onPress={() =>
              // navigation.navigate('kundliDetailes', {kundliData: item})
              navigation.navigate('panchang', {Cname: item?.customer_name, kundli_id: item.kundali_id, place: item.place})
            }
            style={{
  ...
  export default connect(mapStateToProps, null)(PanchangList);
